...
test('OCR relay rejects anonymous use and only forwards allowed Gemini requests',async()=>{
 ...
 try{
  globalThis.fetch=async(input:any,init:any)=>{calls++;assert.equal(String(input),'https://generativelanguage.googleapis.com/v1beta/models/gemini-test:generateContent');assert.equal(init.headers['x-goog-api-key'],env.GEMINI_API_KEY);assert.equal(init.headers.authorization,undefined);assert.equal(init.redirect,'manual');assert.equal(await new Response(init.body).text(),'{"contents":[]}');return new Response('{"candidates":[]}',{status:200});};
