...
test('Branch XLSX has empty import rows, separate examples, dropdowns and usable localized import',async()=>{
 ...
 sheet.getRow(8).values=['Tambah','','Cabang Baru','Mandiri'];sheet.getRow(10).values=['Ubah','DEPOK','Depok Baru',''];const rows=parser.matrixRows(matrix(sheet),'branch');assert.equal(rows.length,2);assert.equal(rows[1]._sourceRow,10);assert.equal(rows[0].action,'ADD');const plan=planBranches(rows,[{id:'DEPOK',name:'Depok',type:'Mandiri'}],'202609');assert.equal(plan.errors.length,0);assert.equal(plan.changes[1].row,10);assert.equal(plan.changes[1].after.type,'Mandiri');
});
