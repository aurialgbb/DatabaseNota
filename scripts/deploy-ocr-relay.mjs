...
const form=new FormData();form.set('metadata',JSON.stringify({main_module:'worker.mjs',compatibility_date:'2026-09-01',placement:{region:'aws:ap-southeast-1'},bindings:[{type:'secret_text',name:'RELAY_SECRET',text:state.secret},{type:'secret_text',name:'GEMINI_API_KEY',text:key},{type:'plain_text',name:'ALLOWED_MODELS',text:[...new Set([primary,fallback,legacyModel])].join(',')}],observability:{enabled:false}}));form.set('worker.mjs',new Blob([fs.readFileSync('ops/cloudflare/ocr-relay.mjs','utf8')],{type:'application/javascript+module'}),'worker.mjs');
await cf('/workers/scripts/'+name,'PUT',form);await cf('/workers/scripts/'+name+'/subdomain','POST',{enabled:true,previews_enabled:false});console.log({deployed:true,url:state.url,applicationConfigChanged:false});
