...
const workspaceCssPath = path.join(publicDir, 'brand', 'workspace.css');
const workspaceSha = fs.existsSync(workspaceCssPath) ? sha(fs.readFileSync(workspaceCssPath)).slice(0, 10) : '1';
const identityCssPath = path.join(publicDir, 'brand', 'identity.css');
const identitySha = fs.existsSync(identityCssPath) ? sha(fs.readFileSync(identityCssPath)).slice(0, 10) : '1';
html = html.replace('</head>', `<link rel="icon" href="/brand/nota-mark.svg" type="image/svg+xml"><link rel="stylesheet" href="/brand/identity.css?v=${identitySha}"><link rel="stylesheet" href="/brand/workspace.css?v=${workspaceSha}"><script src="/brand/menu-banners.js" defer></script></head>`);
