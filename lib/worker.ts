import {runSync} from './sync-actions';
...
export async function workOne(){
 const db=database();
 const job=(await db.query("UPDATE nota_app.jobs SET status='RUNNING',updated_at=now() WHERE id=(SELECT id FROM nota_app.jobs WHERE status='QUEUED' AND kind IN ('STORE_OCR','LEGACY_OCR','LEGACY_TEXT','APPROVAL','SHEET_SYNC') ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING *")).rows[0];
 if(!job)return false;
 ...
 try{
  if(job.kind==='SHEET_SYNC'){await runSync(job);return true;}
  if(job.kind==='APPROVAL'){await runApproval(job);return true;}
