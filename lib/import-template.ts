...
export async function branchTemplate(user:User){requireRole(user,['ADMIN','TAX']);const rows=(await database().query('SELECT id,name,type,active FROM nota_app.branches ORDER BY name')).rows;return {fileName:'template-kelola-cabang-gbb.xlsx',mimeType:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',base64:(await importWorkbook('branch',rows)).toString('base64'),branchCount:rows.length};}
