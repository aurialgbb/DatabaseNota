...
export async function categories(db: Database) {
 return (await db.query('SELECT name,position FROM categories ORDER BY position,name')).rows.map((r:any)=>({id:r.name.replace(/[^A-Z0-9]+/g,'_'),name:r.name,order:r.position,active:true}));
}
export async function receiptDetail(db:Database,user:User,id:string) {
 const row=(await db.query('SELECT r.*,b.name AS branch_name,b.type AS branch_type FROM receipts r JOIN branches b ON b.id=r.branch_id WHERE r.id=$1',[id])).rows[0];
 invariant(row,'NOT_FOUND','Nota tidak ditemukan.',404); requireBranch(user,row.branch_id);
 const items=(await db.query('SELECT * FROM receipt_items WHERE receipt_id=$1 ORDER BY position',[id])).rows.map((i:any)=>({...i.data,id:i.id,description:i.description,category:i.category,quantity:Number(i.quantity),unit:i.unit,amount:Number(i.amount)}));
...
export async function listReceipts(db:Database,user:User,payload:any) {
 ...
 values.push(limit+1);
 const rows=(await db.query('SELECT r.*,b.name AS branch_name,b.type AS branch_type,(SELECT count(*)::int FROM receipt_items i WHERE i.receipt_id=r.id) AS item_count FROM receipts r JOIN branches b ON b.id=r.branch_id WHERE '+clauses.join(' AND ')+" ORDER BY coalesce(r.receipt_date,date '0001-01-01') DESC,r.id DESC LIMIT $"+values.length,values)).rows;
...
export async function writeReceipt(db:Database,user:User,input:any,id:string,status='PENDING',existing=false) {
 ...
 const clean=portalValidateReceiptPayload_(input,names);
 ...
 for(const photoId of photoIds){
  const photo=(await db.query("SELECT * FROM photos WHERE id=$1 AND state='READY'",[photoId])).rows[0];
 ...
 if(existing){
  const previous=(await db.query('SELECT * FROM receipts WHERE id=$1 FOR UPDATE',[id])).rows[0];
  ...
  await db.query('UPDATE receipts SET supplier=$2,receipt_date=$3,total=$4,data=$5,status=$6,version=version+1,updated_at=now() WHERE id=$1',[id,clean.supplier,clean.date,clean.receiptTotal,JSON.stringify({...clean,photoId:photoIds[0]||'',photoIds}),status]);
  await db.query('DELETE FROM receipt_items WHERE receipt_id=$1',[id]);
  await db.query('DELETE FROM receipt_photos WHERE receipt_id=$1',[id]);
 }else{
  await db.query('INSERT INTO receipts(id,branch_id,owner_uid,supplier,receipt_date,total,data,status) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',[id,branchId,user.uid,clean.supplier,clean.date,clean.receiptTotal,JSON.stringify({...clean,photoId:photoIds[0]||'',photoIds}),status]);
 }
 for(let index=0;index<clean.items.length;index++){
  const item=clean.items[index];
  await db.query('INSERT INTO receipt_items(id,receipt_id,position,description,category,quantity,unit,amount,data) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)',[item.id||randomUUID(),id,index,item.description,item.category,item.quantity,item.unit||'',item.amount,JSON.stringify(item)]);
 }
 for(const photoId of photoIds) await db.query('INSERT INTO receipt_photos(receipt_id,photo_id) VALUES($1,$2)',[id,photoId]);
 await db.query('INSERT INTO audit_events(uid,action,entity_id) VALUES($1,$2,$3)',[user.uid,existing?'RECEIPT_UPDATE':'RECEIPT_CREATE',id]);
