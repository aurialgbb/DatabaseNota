...
export async function receiptDetail(db:Database,user:User,id:string) {
 const row=(await db.query('SELECT r.*,b.name AS branch_name,b.type AS branch_type FROM nota_app.receipts r JOIN nota_app.branches b ON b.id=r.branch_id WHERE r.id=$1',[id])).rows[0];
