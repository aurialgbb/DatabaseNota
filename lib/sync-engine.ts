...
export async function persistPlan(jobId:string,step:string,plan:Plan){
 ...
 return transaction(async db=>{
  ...
  for(const expense of [...plan.expenses].sort((a,b)=>a.id.localeCompare(b.id))){const row=(await db.query('SELECT version FROM nota_app.transactions WHERE id=$1 FOR UPDATE',[expense.id])).rows[0];invariant(expense.expectedVersion==null?!row:row?.version===expense.expectedVersion,'TRANSACTION_CONFLICT','Transaksi berubah sebelum sinkronisasi.',409);}
