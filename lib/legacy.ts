...
async function record(db:Database,id:string,kind?:string,expected?:number){const row=(await db.query('SELECT * FROM nota_app.transactions WHERE id=$1 FOR UPDATE',[id])).rows[0];invariant(row&&(!kind||row.kind===kind),'NOT_FOUND','Transaksi tidak ditemukan.',404);if(expected!==undefined)invariant(row.version===Number(expected),'VERSION_CONFLICT','Data sudah berubah. Muat ulang sebelum mengubahnya.',409);return row;}
