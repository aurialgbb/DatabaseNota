...
const profileSelect = 'SELECT p.*, b.name AS branch_name,b.type AS branch_type FROM profiles p LEFT JOIN branches b ON b.id=p.branch_id ';
export async function login(payload: any, verify = firebasePassword) {
  ...
  await transaction(async tx => {
    const active = await tx.query('SELECT active FROM profiles WHERE uid=$1 FOR UPDATE', [row.uid]);
    invariant(active.rows[0]?.active, 'ACCOUNT_DISABLED', 'Akun sudah dinonaktifkan.', 401);
    await tx.query('INSERT INTO sessions(token_hash,uid,expires_at) VALUES($1,$2,$3)', [tokenHash(token), row.uid, new Date(expiresAt)]);
    await tx.query("INSERT INTO audit_events(uid,action) VALUES($1,'LOGIN')", [row.uid]);
    await tx.query('DELETE FROM auth_rate_limits WHERE key=$1', ['login:' + tokenHash(username)]);
  });
...
export async function requireSession(request: Request): Promise<User> {
  ...
  const { rows } = await database().query(profileSelect + ' JOIN sessions s ON s.uid=p.uid WHERE s.token_hash=$1 AND s.expires_at>now() AND p.active=true', [tokenHash(token)]);
  invariant(rows[0], 'SESSION_EXPIRED', 'Sesi telah berakhir. Silakan login kembali.', 401);
  return userFromRow(rows[0]);
}
export async function logout(request: Request) {
  const token = requestToken(request);
  if (token) await database().query('DELETE FROM sessions WHERE token_hash=$1', [tokenHash(token)]);
}
export async function setupStatus() {
  ...
  if (process.env.DATABASE_URL) {
    try { hasAdmin = !!(await database().query("SELECT 1 FROM profiles WHERE role='ADMIN' AND active=true LIMIT 1")).rows.length; databaseReady = true; } catch {}
  }
