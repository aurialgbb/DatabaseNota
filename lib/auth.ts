...
export async function setupStatus() {
 ...
 return {ready:!missing.length&&hasAdmin&&databaseReady,missingProperties:missing,hasAdmin,adminRecordCount:hasAdmin?1:0,databaseReady,recoveryMode:false,bootstrapSecretConfigured:false,diagnostic:databaseReady?'':'Database login belum siap.',features:{ocr:!!process.env.GEMINI_API_KEY&&process.env.OCR_WORKER_ENABLED==='true'&&process.env.OCR_REGION_BLOCKED!=='true',ocrConfigured:!!process.env.GEMINI_API_KEY,sheets:databaseReady&&process.env.OCR_WORKER_ENABLED==='true'&&!!(process.env.GOOGLE_SERVICE_ACCOUNT_JSON||process.env.GOOGLE_SERVICE_ACCOUNT_FILE),sheetsConfigured:!!(process.env.GOOGLE_SERVICE_ACCOUNT_JSON||process.env.GOOGLE_SERVICE_ACCOUNT_FILE)}};
}
