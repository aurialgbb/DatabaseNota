...
export async function createJob(db: Database, user: User, kind: string, payload: any, key: string, branchId = user.branchId) {
  ...
  const inserted = await db.query("INSERT INTO jobs(id,uid,kind,branch_id,request_key,payload_hash,payload) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(uid,kind,request_key) DO NOTHING RETURNING *", [id,user.uid,kind,branchId || null,key,hash,JSON.stringify(payload)]);
  const job = inserted.rows[0] || (await db.query('SELECT * FROM jobs WHERE uid=$1 AND kind=$2 AND request_key=$3', [user.uid,kind,key])).rows[0];
  invariant(job.payload_hash === hash, 'IDEMPOTENCY_CONFLICT', 'Permintaan yang sama memiliki isi berbeda.', 409);
  if (inserted.rows.length) await db.query('INSERT INTO outbox(job_id) VALUES($1)', [id]);
  return job;
}
...
export async function mutateOnce<T>(db: Database, user: User, action: string, key: string, payload: any, fn: () => Promise<T>): Promise<T> {
  ...
  await db.query('INSERT INTO mutations(uid,action,request_key,payload_hash) VALUES($1,$2,$3,$4) ON CONFLICT DO NOTHING', [user.uid,action,key,hash]);
  const row = (await db.query('SELECT * FROM mutations WHERE uid=$1 AND action=$2 AND request_key=$3 FOR UPDATE', [user.uid,action,key])).rows[0];
  ...
  await db.query('UPDATE mutations SET result=$4 WHERE uid=$1 AND action=$2 AND request_key=$3', [user.uid,action,key,JSON.stringify(result)]);
  return result;
}
export async function retryJob(user: User, id: string) {
  return transaction(async db => {
    ...
    await db.query("UPDATE jobs SET status='QUEUED',error_code=NULL,updated_at=now() WHERE id=$1", [id]);
    await db.query('UPDATE outbox SET dispatched_at=NULL,lease_until=NULL,next_attempt_at=now() WHERE job_id=$1',[id]);
...
export async function cancelJob(user: User, id: string) {
  return transaction(async db => {
    ...
    const result = await db.query("UPDATE jobs SET status='CANCELLED',updated_at=now() WHERE id=$1 AND status='QUEUED' RETURNING id", [id]);
