...
Verifikasi deployment Better Auth: login admin dari browser publik berhasil dan daftar akun terbaca dari PostgreSQL; tombol refresh Kelola Akun juga berhasil. Uji HTTP memastikan cookie Secure/HttpOnly/SameSite=Strict, CSRF ditolak (403), registrasi anonim tidak tersedia (401), dan sesi setelah logout ditolak (401). Backup terenkripsi di R2 berhasil dipulihkan menjadi 26 tabel dengan ketiga migrasi. Detail ini memverifikasi login dan pengelolaan akun; tidak menyatakan seluruh 74 tindakan aplikasi lama telah diimplementasikan.

Pembaruan 11 September 2026: minimum password di backend Better Auth, pengelolaan akun, dan UI adalah 6 karakter sesuai permintaan pengguna (maksimum 128). Gemini key sudah diterima endpoint daftar model resmi Google dan tersimpan privat di VPS. Prompt toko dan legacy dibandingkan dengan ekspresi sumber baseline yang hash-nya diverifikasi; keduanya identik untuk kategori yang sama. Migrasi 004 menambahkan 9 kategori bawaan dengan ON CONFLICT DO NOTHING. `features.ocrConfigured` menunjukkan keberadaan key; `features.ocr` tetap false sampai alur pekerjaan OCR tersambung sepenuhnya. Belum ada klaim uji OCR end-to-end atau kesamaan keluaran model.
